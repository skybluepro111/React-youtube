...
module.exports = function(app, db) {
    return {
        ...
        getAllPublish: function (res, Table, element) {
            db[Table].find({ publish: true }, (err, result) => {
                var status = (err) ? 500 : 200;
                var response = (err) ? {status:"DDBB Err:", err} : {[element]: result};
                res.status(status).send(response);
            });
        ...
        getPretsbyAuthorId: function (res, id, Table, element) {
            db[Table].find( {$or:[
                { author: mongoose.Types.ObjectId(id), removed: false }, 
                { 'feedback.to': id },  { 'challenge.to': id }]}, 
                (err, result) => {
                var status = (err) ? 500 : 200;
                var response = (err) ? {status:"DDBB Err:", err} : {[element]: result};
                res.status(status).send(response);
        ...
        setUserAfterPret: function(user_id, object_id, timeLeft, forceDelete) {
            console.log('[setUserAfterPret]', user_id, object_id, timeLeft);
            ...
            db.User.findOne({ _id: user_id }, (err, result) => {
                ...
                if (timeLeft) result.time = timeLeft;
                result.save();
            });
        },
        ...
        findPretAndUpdateUser: function(im, pret_id, pret_duration) {
            var timeleft = im.time + Math.floor(pret_duration / 60);
            this.setUserAfterPret(im._id, pret_id, timeleft, true);
        },
        ...
        setFeedbackToUserId: function (id, pretId, callback) {
            db.User.findOne({ _id: id }, (err, result) => {
                result.feedback.push({ pretId: pretId, completed: false})
                result.save();
                callback((err) ? null : result);
            });
        },
        setChallengeToUserId: function (id, pretId, callback) {
            db.User.findOne({ _id: id }, (err, result) => {
                result.challenge.push({ pretId: pretId, completed: false})
                result.save();
                callback((err) ? null : result);
            });
        },
        ...
        getOrCreateUser: function (uid, username, slug, email, pic) {
            return db.User.findOrCreate({"ref.uid": uid}, {"ref.username": username, "ref.slug": slug, "ref.email": email, "ref.pic": pic});
        }
    };
};